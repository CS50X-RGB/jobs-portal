  ...
  public async createPermission(permissions: PermissionCreate[]) {
    try {
      for (const permission of permissions) {
        const existingRole =
          await this.permissionRepository.findPermissionByName(permission.name);
        if (existingRole) {
  ...
  public async updatePermissions(req: Request, res: Response) {
    try {
      ...
      for (const roleId of Object.keys(roles)) {
        ...
        const updatedRole = await this.roleRepository.togglePermission(
          roleId,
          permissions,
        );
